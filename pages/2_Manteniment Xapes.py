...
def pantalla_afegir_xapa(DB_FILE, opcio):
    ...
    if submitted:
        if not (codi_material and codi_qualitat and codi_acabat):
            ...
            afegir_xapa(DB_FILE, codi_material, codi_qualitat, codi_acabat, espesor, preu_kg) 
...
def pantalla_modificar_xapa(DB_FILE, opcio):
    ...
    if submitted:
        if not(codi_material and codi_qualitat and codi_acabat and espesor):
            ...
            modificar_xapa(DB_FILE, codi_xapa, nou_preu_kg)

# ---- FUNCIONS DEL PROGRAMA ----

def afegir_xapa(DB_FILE, codi_material, codi_qualitat, codi_acabat, espesor, preu_kg):
...
def modificar_xapa(DB_FILE, codi_xapa, nou_preu_kg):
    if nou_preu_kg <= 0:
    ...
    cursor.execute("UPDATE Xapes SET PreuKg = ?, Data = ? WHERE CodiXapa = ?", (nou_preu_kg, nova_data, codi_xapa))
    conexio.commit()
